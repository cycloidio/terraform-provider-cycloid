...
- **Testing uses a dedicated testing environment** — always point `CY_API_URL` at the Cycloid staging/test API, never production. Ensure `CY_API_KEY` and `CY_ORG` correspond to the test environment as well.
